...
def rectangular(low:float, high:float, *, c_m:float=1, no_m:float=0) -> callable:
    ...
    def f_rectangular(x:float) -> float:
        if x < low or high < x:
            return no_m
        else:
            return c_m

    return f_rectangular
...
def bounded_exponential(k=0.1, limit=1):
    ...
    assert limit > 0
    assert k > 0
    def f_bounded_exponential(x):
        try: 
            return limit - limit/exp(k*x)
        except OverflowError:
            return limit
    return f_bounded_exponential
...
def triangular_sigmoid(low, high, c=None):
    ...
    def f_triangular_sigmoid(x):
        if x <= c:
            return left_slope(x)
        else:
            return right_slope(x)

    return f_triangular_sigmoid
